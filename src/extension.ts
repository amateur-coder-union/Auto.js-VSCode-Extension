...
class Extension {
  ...
  selectDevice(callback) {
    ...
    vscode.window.showQuickPick(names).then(select => {
  ...
  runOn(
    ...
    if (
      target instanceof oldAutojs.Device ||
      target instanceof oldAutojs.AutoJsDebugServer
    ) {
      target.send({
        type: 'command',
        command: 'run',
